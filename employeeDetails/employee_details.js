const employees = [
    ...
    { id: 2, name: 'Alice Smith', age: 28, department: 'HR', salary: 45000 ,specialization:'TS'},
    { id: 3, name: 'Bob Johnson', age: 35, department: 'Finance', salary: 60000,specialization:'PHP' },
...
function findEmployeeById(employeeSpe) {
    const foundEmployee = employees.find(employee => employee.specialization === employeeSpe);
    if (foundEmployee) {
    document.getElementById('employeesDetails').innerHTML =`<p>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary}</p>`;
    }
    else{
